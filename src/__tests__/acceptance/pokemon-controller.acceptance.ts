...
import {cleanDatabase, givenEmptyDatabase, givenPokemon} from '../helpers/datasource.helper';
...
describe('Pokemon controller should ', () => {
  ...
  it('retrieve page 1 of size 2 when filter paginated', async () => {
    ...
    const response = await client.get(`/pokemon?page=${queryPage}&size=${querySize}`);
